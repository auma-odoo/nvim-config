    ...
    opts = {
      ...
      handlers = {
        python = function(source_name)
          ...
          local database_picker = function(callback) custom_picker("Choose the database", get_dababase_list(), callback) end

          local ask_test_tags = function(callback)
            vim.ui.input({ prompt = "Test tags" }, function(input) callback(input) end)
          end

          local ask_modules = function(callback)
            vim.ui.input({ prompt = "Modules to initialize" }, function(input) callback(input) end)
          end

          local ask_name_database = function(callback)
            vim.ui.input({ prompt = "Name of the database" }, function(input) callback(input) end)
          end
          ...
          local ask_yes_no_question = function(prompt, callback)
            vim.ui.select({ "No", "Yes" }, { prompt = prompt }, function(choice)
              ...
              if choice == "Yes" then returned_boolean = true end
              callback(returned_boolean)
            end)
          end

          local ask_if_new_database = function(callback)
            ask_yes_no_question("Do you want to create a new database?", function(answer) callback(answer) end)
          end
          ...
          local ask_format_output = function(callback)
            vim.ui.select(
              ...
              function(selection)
                local filetype = "json"
                if selection == "flamegraph" then filetype = "svg" end
              ...
              args = function()
                return coroutine.create(function(coro)
                  ask_if_new_database(function(answer)
                    if answer then
                      ask_name_database(
                        function(database_name) coroutine.resume(coro, get_normal_arg(database_name)) end
                      )
                    else
                      database_picker(function(selection) coroutine.resume(coro, get_normal_arg(selection)) end)
                    end
                  end)
                end)
              end,
                    ...
                    ask_if_new_database(function(answer)
                      if answer then
                        ...
                        ask_name_database(function(database_name)
                          ask_modules(
                            function(modules) coroutine.resume(coro, get_test_args(modules, tags, database_name)) end
                          )
                        end)
                      else
